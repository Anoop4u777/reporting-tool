...
const CheckingsAccount = ({ data }) => {
  const checkingsAccounts = data.filter((account) =>
    account.account_type.toLowerCase().includes("checking")
  );

  return (
    <div className=" p-3 bg-[#eef1f5] rounded-lg text-black shadow-md w-fit">
      <h3 className="text-black mb-3 text-base font-bold">
        Checkings Accounts
      </h3>
      <div className="flex flex-col gap-3">
        {checkingsAccounts.map((account, index) => (
...
export default CheckingsAccount;
